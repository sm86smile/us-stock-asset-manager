...
DEFAULT_STRATEGY_TICKER_ROWS = [
    {"strategy_key": "laa", "strategy_name": "LAA", "group_key": "fixed", "group_name": "고정자산 75%", "role_key": "laa_fixed_1", "role_name": "고정자산 1", "sort_order": 1, "ticker": "IWD", "asset_class": "미국 대형 가치주"},
    {"strategy_key": "laa", "strategy_name": "LAA", "group_key": "fixed", "group_name": "고정자산 75%", "role_key": "laa_fixed_2", "role_name": "고정자산 2", "sort_order": 2, "ticker": "GLD", "asset_class": "금"},
    {"strategy_key": "laa", "strategy_name": "LAA", "group_key": "fixed", "group_name": "고정자산 75%", "role_key": "laa_fixed_3", "role_name": "고정자산 3", "sort_order": 3, "ticker": "IEF", "asset_class": "미국 중기국채"},
    {"strategy_key": "laa", "strategy_name": "LAA", "group_key": "variable", "group_name": "변동자산 25%", "role_key": "laa_variable_risk", "role_name": "조건 미충족 시 위험자산", "sort_order": 4, "ticker": "QQQ", "asset_class": "나스닥100"},
    {"strategy_key": "laa", "strategy_name": "LAA", "group_key": "variable", "group_name": "변동자산 25%", "role_key": "laa_variable_defensive", "role_name": "조건 충족 시 방어자산", "sort_order": 5, "ticker": "SHY", "asset_class": "미국 단기국채"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "attack", "group_name": "공격형", "role_key": "vaa_attack_1", "role_name": "공격형 1", "sort_order": 1, "ticker": "SPY", "asset_class": "미국 S&P500"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "attack", "group_name": "공격형", "role_key": "vaa_attack_2", "role_name": "공격형 2", "sort_order": 2, "ticker": "EFA", "asset_class": "선진국 주식(미국 제외)"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "attack", "group_name": "공격형", "role_key": "vaa_attack_3", "role_name": "공격형 3", "sort_order": 3, "ticker": "EEM", "asset_class": "신흥국 주식"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "attack", "group_name": "공격형", "role_key": "vaa_attack_4", "role_name": "공격형 4", "sort_order": 4, "ticker": "AGG", "asset_class": "미국 종합채권"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "safe", "group_name": "안전자산", "role_key": "vaa_safe_1", "role_name": "안전자산 1", "sort_order": 5, "ticker": "LQD", "asset_class": "미국 투자등급 회사채"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "safe", "group_name": "안전자산", "role_key": "vaa_safe_2", "role_name": "안전자산 2", "sort_order": 6, "ticker": "IEF", "asset_class": "미국 중기국채"},
    {"strategy_key": "vaa", "strategy_name": "VAA 공격형", "group_key": "safe", "group_name": "안전자산", "role_key": "vaa_safe_3", "role_name": "안전자산 3", "sort_order": 7, "ticker": "SHY", "asset_class": "미국 단기국채"},
    {"strategy_key": "odm", "strategy_name": "오리지널 듀얼 모멘텀", "group_key": "roles", "group_name": "비교/선택 자산", "role_key": "odm_us_equity", "role_name": "미국 주식 기준자산", "sort_order": 1, "ticker": "SPY", "asset_class": "미국 S&P500"},
    {"strategy_key": "odm", "strategy_name": "오리지널 듀얼 모멘텀", "group_key": "roles", "group_name": "비교/선택 자산", "role_key": "odm_intl_equity", "role_name": "해외 주식 비교자산", "sort_order": 2, "ticker": "EFA", "asset_class": "선진국 주식(미국 제외)"},
    {"strategy_key": "odm", "strategy_name": "오리지널 듀얼 모멘텀", "group_key": "roles", "group_name": "비교/선택 자산", "role_key": "odm_cash", "role_name": "현금성 절대모멘텀 기준", "sort_order": 3, "ticker": "BIL", "asset_class": "초단기 미국 국채"},
    {"strategy_key": "odm", "strategy_name": "오리지널 듀얼 모멘텀", "group_key": "roles", "group_name": "비교/선택 자산", "role_key": "odm_bond", "role_name": "방어 채권자산", "sort_order": 4, "ticker": "AGG", "asset_class": "미국 종합채권"},
]
...
def normalize_strategy_ticker_editor(edited_df: pd.DataFrame, current_df: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    """화면에서 수정한 티커/자산군 설명을 고정된 전략 역할 구조에 맞춰 정리합니다."""
    ...
    for default_row in DEFAULT_STRATEGY_TICKER_ROWS:
        ...
        old_asset_class = str(current_row.get("asset_class", default_row["asset_class"]) or "").strip()
        new_asset_class = str(edited_row.get("asset_class", old_asset_class) or "").strip()

        # 티커만 바꾸고 기존 자산군 문구를 그대로 둔 경우, 잘못된 설명이 남지 않도록 자동 보정합니다.
        if new_ticker != old_ticker and new_asset_class == old_asset_class:
            new_asset_class = DEFAULT_ETF_LABELS.get(new_ticker, f"{new_ticker} 사용자 지정 ETF")
        if not new_asset_class:
            new_asset_class = DEFAULT_ETF_LABELS.get(new_ticker, f"{new_ticker} 사용자 지정 ETF")

        row = dict(default_row)
        row["ticker"] = new_ticker
        row["asset_class"] = new_asset_class
        row["updated_at"] = updated_at
        rows.append(row)

    normalized = pd.DataFrame(rows, columns=STRATEGY_TICKER_COLUMNS)
    return normalized, validate_strategy_tickers(normalized)
...
with tab_strategy:
    ...
    with st.expander("전략별 ETF 티커 설정", expanded=False):
        st.caption(
            ...
            "티커를 바꾸면 매수 전략 요약, 계산 방식, 최근 리밸런싱일 대상, API 신규 조회 및 전략 계산이 모두 새 설정을 사용합니다."
        )
        st.warning("티커 변경 후에는 기존 rebalance_basis RAW에 새 티커 데이터가 없을 수 있으므로, 현재 포트폴리오 기준으로 신규 조회 계산을 한 번 실행하세요.")
        ticker_editor = st.data_editor(
            strategy_ticker_df[["strategy_name", "group_name", "role_key", "role_name", "ticker", "asset_class"]],
            key="strategy_ticker_editor",
            use_container_width=True,
            hide_index=True,
            num_rows="fixed",
            disabled=["strategy_name", "group_name", "role_key", "role_name"],
            column_config={
                ...
                "role_name": st.column_config.TextColumn("역할"),
                "ticker": st.column_config.TextColumn("ETF 티커", help="Alpha Vantage에서 조회 가능한 미국 ETF 티커를 입력하세요."),
                "asset_class": st.column_config.TextColumn("자산군 설명", help="앱의 전략 요약과 결과표에 표시할 설명입니다."),
            },
        )
        ...
        if ticker_save_col.button("전략별 ETF 티커 저장", type="primary"):
            ...
            if ticker_errors:
                ...
                st.success("strategy_tickers 시트에 저장했습니다. 새 설정을 앱 전체에 반영합니다.")
                st.rerun()
        ticker_info_col.info(f"현재 API 신규 조회 대상은 중복을 제거한 {len(ALL_TICKERS)}개 ETF입니다: {', '.join(ALL_TICKERS)}")
