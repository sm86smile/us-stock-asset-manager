...
st.caption("Google Sheets에 현금/매매일지/마지막 리밸런싱 결과를 저장하고, 최신 가격은 사용자가 버튼을 눌렀을 때만 Alpha Vantage로 조회합니다.")
...
with tab_strategy:
    ...
    if run:
        ...
        saved_rebalance_plan_df = load_saved_rebalance_plan()
        st.success("리밸런싱 결과를 Google Sheets의 rebalance_plan 시트에 저장했습니다. 새로 계산하기 전까지 이 결과가 유지됩니다.")
