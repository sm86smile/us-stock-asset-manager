...
BUYSELLHOLD 주문안 계산
Google Sheets 저장 구조
rebalance_plan 컬럼 설명
rebalance_basis 컬럼 설명
화면에서 확인할 수 있는 결과
권장 사용 순서
오류 메시지와 해결 방법
계산상 주의사항
...
따라서 정상적인 한 번의 신규 계산에서는 11개 ETF × 1회 = 총 11회가 호출됩니다.
...
서비스 계정 인증정보
Google Spreadsheet
GitHub 계정과 Streamlit Community Cloud 계정 — 온라인 배포 시
---
7. 로컬 설치 및 실행
7.1 프로젝트 폴더 생성
```bash
...
mkdir .streamlit
```
`.streamlit/secrets.toml` 파일을 작성합니다. 자세한 예시는 11. Streamlit Secrets 설정을 참고하십시오.
7.7 앱 실행
소스 파일명을 유지한 경우:
```bash
...
9.5 Google Sheet 공유
생성한 Google Spreadsheet의 `공유` 버튼을 누르고 서비스 계정 이메일을 편집자로 추가합니다.
...
12.2 배포
Streamlit Community Cloud에 로그인합니다.
`Create app`을 선택합니다.
GitHub 저장소와 브랜치를 선택합니다.
메인 Python 파일 경로를 지정합니다.
앱의 Secrets 설정 화면에 `secrets.toml` 내용을 붙여 넣습니다.
배포를 실행합니다.
12.3 배포 후 확인
...
앱을 열면 다음 순서로 초기화됩니다.
Streamlit 페이지 설정
Secrets에서 API Key와 Sheet ID 확인
평가 기준일 입력
`settings` 시트에서 USD/KRW 환율 조회
`trades` 시트 불러오기
`cash` 시트의 마지막 잔고 불러오기
`rebalance_plan`의 마지막 주문안 불러오기
`rebalance_basis`의 마지막 근거와 RAW 불러오기
매매일지로 현재 보유수량 계산
전략별 최근 매매일 계산
저장 RAW에서 최근 월봉 `close`를 복원
복원한 가격을 Streamlit 세션 가격에 저장
현재 총자산 계산
자산 탭과 리밸런싱 탭 표시
...
따라서 `cash` 시트는 입출금 거래원장이 아니라 잔고 스냅샷 기록입니다.
예시:
updated_at	cash_usd	cash_krw	memo
...
저장 RAW에서 복원한 가격 또는 현재 세션의 수동 가격
가격이 없으면 평균매수가
평균매수가도 없으면 평가 제외
평가 기준은 다음 값으로 표시됩니다.
`REBALANCE_BASIS_RAW`
...
22.2 앱 재실행 시 복원
`rebalance_basis`에서 `section = ETF 월봉 RAW` 행만 필터링
티커별로 DataFrame 복원
평가 기준일 이하 최근 `close` 선택
세션의 `latest_quotes_df`에 저장
현재 자산평가에 사용
22.3 수동 투자금 계산 시 복원
...
35. 권장 사용 순서
최초 설정
Alpha Vantage API Key를 준비합니다.
Google Cloud 서비스 계정을 만듭니다.
Google Sheets API를 활성화합니다.
Google Spreadsheet를 생성합니다.
서비스 계정 이메일을 Sheet 편집자로 공유합니다.
`secrets.toml`을 설정합니다.
앱을 실행합니다.
`settings` 시트에 환율 설정을 입력합니다.
최초 데이터 입력
자산 탭에서 USD와 KRW 현금잔고를 저장합니다.
보유 중인 종목의 매수·매도·입출고 내역을 입력합니다.
종목별 보유수량을 실제 증권계좌와 비교합니다.
전략 외 종목은 필요하면 수동가격을 입력합니다.
최초 리밸런싱 계산
평가 기준일을 확인합니다.
`현재 포트폴리오 총자산 사용`을 선택합니다.
데이터 조회기간을 선택합니다.
진행 중인 월 제외 여부를 결정합니다.
LAA 조건을 O 또는 X로 선택합니다.
VAA 0점 처리 옵션을 결정합니다.
LAA·VAA·ODM 비중을 입력합니다.
`11개 ETF 신규 조회 후 전략 계산`을 누릅니다.
API 11회 조회가 완료될 때까지 화면 진행률을 확인합니다.
전략 선택 결과를 확인합니다.
최종 주문안을 검토합니다.
`rebalance_basis`의 계산 근거를 확인합니다.
필요한 경우 CSV를 다운로드합니다.
실제 매매 후
실제 체결수량과 체결가격을 매매일지에 입력합니다.
현금잔고를 실제 계좌잔고로 업데이트합니다.
종목별 수량과 현금이 일치하는지 확인합니다.
API를 사용하지 않는 재계산
`수동 입력`을 선택합니다.
KRW 또는 USD를 선택합니다.
총 투자금을 입력합니다.
전략 조건과 비중을 수정합니다.
`저장된 ETF RAW로 전략 계산(API 미호출)`을 누릅니다.
---
36. 오류 메시지와 해결 방법
